...
import { getQwenConfig } from "../../translation/qwen";
...
export const qwenDefinitionProvider: DefinitionProvider = {
  id: "qwen",
    ...
    try {
      ...
      if (!response.ok) {
        ...
        return createDefinitionError(
          errorCode,
          `Definition unavailable (HTTP ${response.status}) via Qwen.${details}`
        );
      }
