...
import { getVolcengineConfig } from "../../translation/volcengine";
...
export const volcengineDefinitionProvider: DefinitionProvider = {
  id: "volcengine",
    ...
    try {
      ...
      if (!response.ok) {
        ...
        return createDefinitionError(
          errorCode,
          `Definition unavailable (HTTP ${response.status}) via Volcengine.${details}`
        );
      }
