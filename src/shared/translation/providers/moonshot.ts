...
const parseTranslation = (
  text: string
): { translatedWord: string; translatedDefinition?: string | null } | null => {
...
export const moonshotProvider: TranslationProvider = {
  id: "moonshot",
  translate: async (
    ...
    try {
      ...
      if (!response.ok) {
        ...
        return createTranslationError(
          errorCode,
          `Translation unavailable (HTTP ${response.status}) via Moonshot.${details}`
        );
      }
