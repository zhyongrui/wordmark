...
import { deepseekProvider } from '../../../src/shared/translation/providers/deepseek';
import { clearDeepSeekConfig, writeDeepSeekConfig } from '../../../src/shared/translation/deepseek';
...
describe('deepseek translation provider', () => {
  ...
  it('sends minimal payload (word + optional definition only) and parses JSON response', async () => {
    ...
    const response = await deepseekProvider.translate(
      {
        word: 'hello',
  ...
  it('returns not_configured when endpoint or model is missing', async () => {
    ...
    const response = await deepseekProvider.translate({ word: 'hello', definition: null, targetLang: 'zh' }, 'test-key');
